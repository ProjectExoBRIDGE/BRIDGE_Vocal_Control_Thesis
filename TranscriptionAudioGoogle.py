#AUTOMATIC AUDIO TRANSCRIPTION WITH GOOGLE
...
# The words are recognized with Google
...
file = sr.AudioFile(file_path + names_array[0])  # loading file - 0
...
try:  # Try to listen to the file - 0
    ...
    text = r.recognize_google(audio, language='it')  # saving the transcription in 'text'
    print('The test file got: ', text)  # printing separated words
...
file_excel = xlsxwriter.Workbook('Matrix_Google.xlsx')
...
with open('timesGoogle', 'wb') as file:           # Open the file in binary mode
    pickle.dump(process_time, file)             # Serialize and write the variable to the file

print("The data", 'timesGoogle', " has been saved successfully.")
