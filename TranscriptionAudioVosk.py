...
# Also, the script saves the processing time in an array, then saves the array in a file.
# If some files have no audio recognizable, the script saves '-100' in the array cell representing that file.
...
########################################################################################################
# LISTENING TO A TEST - FILE IN ORDER TO TRY THE SYSTEM AND IMPORT THE MODEL
# This phase is needed because speech recognition libraries can have very high first recognition times
# This is mainly due to the creation of the object that contains the model
# and therefore the very first detection contains more operations than the subsequent ones.
# For that reason, the script run a detection with the first element of the names_array file, but it doesn't consider
# it as part of the confusion matrix building.

file = sr.AudioFile(file_path + names_array[0])  # loading file - 0 (first detection)

try:  # Try to listen to the file - 0

    with file as source:
        audio = r.record(source)  # listening audio of file-0
    ...
    print('The test file got: ', text)  # printing  words

except:                           # if it cannot, ask user to change the test-file...
    print("NO AUDIO RECOGNIZABLE IN THIS FILE, PLEASE CHANGE TEST FILE")
    sys.exit()                    # and stop the program
...
# Initializing process time array
...
for i in range(len(names_array)):                    # setting the file - i
    ...
    while (found == 0 and j < len(text_words)):  # unless it finds the command or exceeds the text_words array
        ...
        try:                                     # try to find the command in the word -j
            recognized_command = get_close_matches(text_words[j], commands, n=1, cutoff=0.7)[0]
    ...
    while (found == 0 and j < len(text_words)):  # unless it finds the entity or exceeds the text_words array
        ...
        try:                                     # try to find the entity in the word -j
            recognized_entity = get_close_matches(text_words[j], entities, n=1, cutoff=0.7)[0]
            found = 1                            # if it can... found = 1
...
########################################################################################################
# SAVING THE COMMAND MATRIX IN AN EXCEL FILE

# 1. Create a file

file_excel = xlsxwriter.Workbook('Matrix_Vosk.xlsx')

# 2. Create a sheet for commands matrix
...
# SAVING THE ENTITY MATRIX IN AN EXCEL FILE
...
with open('timesVosk', 'wb') as file:           # Open the file in binary mode
    pickle.dump(process_time, file)             # Serialize and write the variable to the file

print("The data", 'timesVosk', " has been saved successfully.")
